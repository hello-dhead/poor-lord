...
public class GECubixOrbitCamera : MonoBehaviour
{
	...
	void Start()
	{
		...
		SetTargetToOrbit(m_Target.gameObject);
	}
	...
	void UpdatePosition()
	{
		...
		if (m_Y>90)
			m_Y = Mathf.Clamp(m_Y, 290, 360);
		else if (m_Y>70)
			m_Y = Mathf.Clamp(m_Y, 0, 70);

		Quaternion rotation = Quaternion.Euler(m_Y, m_X, 0);


		Vector3 direction = new Vector3(0.0f, 0.0f, -m_Distance);
		Vector3 position = (rotation * direction) + m_OrbitTarget;

		transform.rotation = rotation;
		transform.position = position;
		this.gameObject.transform.LookAt(m_OrbitTarget);
	}
